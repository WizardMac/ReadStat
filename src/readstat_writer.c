...
void readstat_label_double_value(readstat_label_set_t *label_set, double value, const char *label) {
    ...
    if (label && strlen(label)) {
        new_value_label->label = malloc(strlen(label));
        strcpy(new_value_label->label, label);
    }
}
...
void readstat_label_string_value(readstat_label_set_t *label_set, const char *value, const char *label) {
    ...
    if (label && strlen(label)) {
        new_value_label->label_len = strlen(label);
        new_value_label->label = malloc(new_value_label->label_len);
        strncpy(new_value_label->label, label, new_value_label->label_len);
    }
}
